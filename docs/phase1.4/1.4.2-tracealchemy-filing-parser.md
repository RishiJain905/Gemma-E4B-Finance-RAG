# 1.4.2 — TraceAlchemy Filing Parser (Model-as-Parser)

## Objective

Create the `TraceAlchemyFilingParser` — the core "model-as-parser" component that sends SEC filing text to the local TraceAlchemy model and extracts structured financial facts (revenue, net income, EPS, operating income, etc.) from 10-K/10-Q filings.

## Why This Matters

This is the **signature capability** of the TraceAlchemy system — using a fine-tuned Gemma 4 E4B model to parse dense regulatory text into queryable structured data. The parser bridges unstructured documents → structured facts, which is what makes the hybrid RAG approach powerful.

The model's `fact_extraction` task is already defined in `configs/model.yaml` (temperature=0.1, max_tokens=1024).

## Step 1: Define the Extraction Schema

The parser needs a well-defined schema for what facts to extract from each filing type.

Create `src/sec/__init__.py` (update, file may already exist from 1.4.1):
...
Create `src/sec/filing_parser.py`:
