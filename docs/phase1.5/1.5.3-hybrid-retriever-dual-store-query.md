# 1.5.3 — Hybrid Retriever (Dual-Store Query)

## Objective

Create the `Retriever` — the module that performs **dual retrieval** against both storage backends (SQLite for structured facts, ChromaDB for semantic documents) based on the parsed intent from 1.5.2.

## Why This Matters

The retriever is the bridge between "what the user wants" (intent) and "what we have" (data). It decides:
- **Which store(s) to query** based on question type
- **How to query** — exact match for facts, semantic search for documents
- **How to rank** — prioritize ticker-specific results, then broaden
- **How to merge** — combine facts + documents into a unified context for the prompt

A good retriever means the model gets exactly the right context — not too little (hallucination risk) and not too much (context window waste).

## Step 1: Create the Retriever Module

Create `src/middleware/retriever.py`:
...
intent = parser.parse('What is NVDA revenue?')
assert retriever._select_strategy(intent['question_type'], intent['ticker'], intent['metrics']) == 'facts_only'

intent = parser.parse('How is the market sentiment on AMD?')
assert retriever._select_strategy(intent['question_type'], intent['ticker'], intent['metrics']) == 'documents_only'

intent = parser.parse('Compare NVDA and AMD')
assert retriever._select_strategy(intent['question_type'], intent['ticker'], intent['metrics']) == 'comparison'

intent = parser.parse('Tell me about AI chips')
assert retriever._select_strategy(intent['question_type'], None, []) == 'broad'

# Test multi-ticker extraction
...
- [ ] `facts_only` strategy used for fact_lookup with known ticker + metrics
- [ ] `documents_only` strategy used for sentiment/news/risk questions
- [ ] `hybrid` strategy used for general questions with a ticker
- [ ] `comparison` strategy extracts multiple tickers from the query
- [ ] `broad` strategy used when no ticker is detected
- [ ] SQLite facts are returned with metric, value, ticker, source_type
- [ ] ChromaDB documents include id, text, and metadata
- [ ] Results are deduplicated (no duplicate metrics in facts list)
- [ ] Empty results are handled gracefully (empty lists, no crashes)
