# 1.5.5 — Integration Test & Verification

## Objective

Create a comprehensive integration test suite for the Phase 1.5 FastAPI middleware — verify end-to-end query flow from HTTP request through intent parsing → retrieval → prompt augmentation → model call → response, and validate that the middleware integrates cleanly with the existing Phase 1.3/1.4 storage and data.

## Why This Matters

The middleware is the user-facing layer — if it breaks, the entire system is unusable. Unlike the ingestion pipeline (which can fail silently and retry), the query endpoint needs to be reliable, fast, and correct. This test suite catches:

- Import errors or missing modules in the middleware package
- Intent parser regressions (ticker detection, metric extraction)
- Retriever strategy selection bugs
- Prompt augmentation formatting issues
- API endpoint contract violations (wrong status codes, missing fields)
- Integration issues between middleware and storage backends
- Edge cases: empty DB, unknown tickers, malformed questions, very long queries
...
Usage:
    pytest tests/test_middleware.py -v              # All mocked tests
    pytest tests/test_middleware.py -v -x           # Stop on first failure
    pytest tests/test_middleware.py -k "intent"     # Run intent parser tests only

NOTE: Live model tests (marked @pytest.mark.live) require:
    - llama-server running on :8087
    - Run with: pytest tests/test_middleware.py -v --live
"""
...
class TestRetriever:
    ...
    def test_strategy_selection(self, qtype, ticker, metrics, expected_strategy, tmp_path):
    ...
    def test_retrieve_with_seeded_data(self, tmp_path):
    ...
    def test_multi_ticker_extraction(self, tmp_path):
...
class TestMiddlewareIntegration:
    ...
    def test_prompt_augmenter_integration(self, seeded_store):
        ...
        # Verify the prompt contains the seeded data
    ...
    def test_unknown_ticker_graceful(self, seeded_store):
        ...
        # Should handle gracefully — either no data found or empty results
        assert len(prompt) > 0
```
...
pytest tests/test_middleware.py -v -k "intent"
pytest tests/test_middleware.py -v -k "retriever"
pytest tests/test_middleware.py -v -k "prompt"
pytest tests/test_middleware.py -v -k "api or endpoint"

# Run integration tests
pytest tests/test_middleware.py -v -k "integration"

# Run with coverage
pip install pytest-cov
pytest tests/test_middleware.py -v --cov=src.middleware
```
...
# 3. Search (bypasses model)
curl -X POST http://localhost:8000/search \
  -H "Content-Type: application/json" \
  -d '{"query": "NVDA revenue", "n_results": 5}'

# 4. Query (full pipeline — requires llama-server)
curl -X POST http://localhost:8000/query \
  -H "Content-Type: application/json" \
  -d '{"question": "What is NVDA revenue?"}'

# 5. Query with ticker override
curl -X POST http://localhost:8000/query \
  -H "Content-Type: application/json" \
  -d '{"question": "What is revenue?", "ticker": "AMD"}'

# 6. Comparison query
curl -X POST http://localhost:8000/query \
  -H "Content-Type: application/json" \
  -d '{"question": "Compare NVDA and AMD revenue"}'

# 7. Sentiment query
curl -X POST http://localhost:8000/query \
  -H "Content-Type: application/json" \
  -d '{"question": "What is the market sentiment on NVDA?"}'
```

## Verification Checklist

- [ ] All unit tests pass (intent parser, retriever, prompt augmenter)
- [ ] All API endpoint tests pass (health, search, query, validation)
- [ ] Integration tests pass with seeded data
- [ ] Intent parser correctly handles all question types
- [ ] Retriever selects correct strategy for each question type
- [ ] Prompt augmenter produces well-structured prompts with citations
- [ ] API returns proper error codes for invalid requests (422)
- [ ] Health endpoint reports storage status correctly
- [ ] Search endpoint returns data from both stores
- [ ] Query endpoint returns structured response with answer + citations
- [ ] Empty database is handled gracefully (no crashes)
- [ ] Unknown tickers are handled gracefully
