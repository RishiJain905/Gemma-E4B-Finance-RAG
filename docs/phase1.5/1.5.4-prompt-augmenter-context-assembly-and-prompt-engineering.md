# 1.5.4 — Prompt Augmenter (Context Assembly & Prompt Engineering)

## Objective

Create the `PromptAugmenter` — the module that takes the retrieved facts + documents and assembles them into a structured, grounded prompt for the TraceAlchemy model. This is where the "augmented" in RAG happens.

## Why This Matters

The quality of the answer depends almost entirely on the quality of the prompt. A well-structured augmented prompt:
- **Prevents hallucination** — the model has the exact data it needs
- **Enables citations** — sources are embedded inline so the model can reference them
- **Handles missing data gracefully** — the prompt tells the model what to do when data is absent
- **Adapts to question type** — comparison questions get a different prompt structure than fact lookups

This is the final step before the model call — get this right and the answers are sharp, grounded, and trustworthy.

## Step 1: Create the Prompt Augmenter Module

Create `src/middleware/prompt_augmenter.py`:
...
# Test with facts
intent = {'ticker': 'NVDA', 'metrics': ['total_revenue'], 'question_type': 'fact_lookup'}
retrieval = {
    'facts': [
...
prompt = augmenter.build_prompt('What is NVDA revenue?', intent, retrieval)
...
print('Fact prompt OK')

# Test with documents
retrieval2 = {
    'facts': [],
    'documents': [
        {'id': 'sec_10k/NVDA/10-K-2025', 'text': 'NVIDIA reported strong growth...',
         'ticker': 'NVDA', 'source': '10-K', 'date': '2025-03-15',
         'metadata': {'ticker': 'NVDA', 'source': '10-K'}},
    ],
    'ticker': 'NVDA',
}
prompt2 = augmenter.build_prompt('What is the outlook for NVDA?',
                                 {'ticker': 'NVDA', 'question_type': 'sentiment'}, retrieval2)
assert 'Retrieved Documents' in prompt2
assert 'NVIDIA reported' in prompt2
print('Document prompt OK')

# Test empty retrieval
prompt3 = augmenter.build_prompt('What about AAPL?',
                                  {'ticker': 'AAPL', 'question_type': 'general'},
                                  {'facts': [], 'documents': [], 'ticker': 'AAPL'})
assert 'No data was found' in prompt3
print('Empty retrieval prompt OK')

# Test truncation
long_docs = [
    {'id': f'doc_{i}', 'text': 'X' * 5000, 'ticker': 'NVDA',
     'source': '10-K', 'metadata': {'ticker': 'NVDA', 'source': '10-K'}}
    for i in range(10)
]
long_prompt = augmenter.build_prompt('Test?', {'ticker': 'NVDA', 'question_type': 'general'},
                                     {'facts': [], 'documents': long_docs, 'ticker': 'NVDA'})
truncated = augmenter.truncate_if_needed(long_prompt, max_tokens=2000)
assert len(truncated) < len(long_prompt)
print('Truncation OK')

print('All prompt augmenter tests passed!')
"
```

## Verification Checklist

- [ ] Fact-lookup prompt includes structured facts with metric, value, period, source
- [ ] Document prompt includes document text with metadata (ticker, source, date)
- [ ] Empty retrieval generates a "no data found" note
- [ ] Question-type-specific instructions are included (comparison, sentiment, etc.)
- [ ] Output format instructions are always present
- [ ] Source citations use [Source: type/ticker] notation
- [ ] Long prompts are truncated gracefully (documents first, then facts)
- [ ] Token estimation is within reasonable bounds
- [ ] System instruction includes rules about using only provided context
