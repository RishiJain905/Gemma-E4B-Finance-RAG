...
- [x] 2.3.5.1 source-aware-live-trace-contract — **done:** additive allowlisted source-aware evidence metadata (schema stays v1), provider/publisher + primary/corroborating distinctions, shared `stage:route` identity preserved incl. fallback merge, mixed SEC/news/macro/market golden fixture; 1615 offline tests, VERIFY: PASS. — spec: `docs/phase2.3/2.3.5-graph-and-corpus-explorer-scale/2.3.5.1-source-aware-live-trace-contract.md` — extend Live Trace's `question -> plan/subquery -> retrieval/tool -> evidence -> source -> answer/citation` model to explain the expanded source set for one executed query, without provider-specific graph schemas and without turning scheduler/ingestion runs into query traces (that belongs in Corpus Explorer).
- [x] 2.3.5.2 corpus-explorer-information-architecture — **done:** aggregation-first landing + persistent facet rail with authoritative counts (`/graph/api/corpus/aggregates`, SQLite-only), keyboard-navigable inventory, URL-hash deep links (no storage), per-security badges, 7 presets; Phase 2.2 folding/LOD/CSP preserved; index/sector counts deferred to 2.3.5.3 registry join; 1630 offline tests, VERIFY: PASS. — spec: `docs/phase2.3/2.3.5-graph-and-corpus-explorer-scale/2.3.5.2-corpus-explorer-information-architecture.md` — reorganize Corpus Explorer around an aggregation-first landing view (market universe → sector/index/source groups → on-demand expansion) that generalizes the Phase 2.2 freshness fold/aggregate pattern instead of rendering thousands of tickers/documents directly.
- [x] 2.3.5.3 bounded-projection-ui-performance-and-accessibility — **done:** indexed SQLite aggregate queries + new corpus node/relation vocabulary, additive facets/groups/items endpoints (paged, revision-aware, read-only), 600-security/100k-item deterministic scale fixture with warm p95 asserted in-gate (<250ms overview/facet, <300ms filtered page), stage-rail/scrubber guards + a11y contract extended; 1651 offline tests, VERIFY: PASS. — spec: `docs/phase2.3/2.3.5-graph-and-corpus-explorer-scale/2.3.5.3-bounded-projection-ui-performance-and-accessibility.md` — extend the read-only corpus projection and graph APIs (indexed bounded aggregate queries, Cytoscape zoom/pan-aligned stage lanes, rAF-coalesced scrubbing) so the new information architecture stays responsive, bounded, and accessible at Phase 2.3 scale.

## 2.3.6 — Integration, migration, and evaluation

- [x] 2.3.6.1 schema-config-and-backward-compatible-migration — **done:** five ordered checksum-protected migrations + `schema_migrations` (stdlib only), canonical/inline/empty/2.2 schema convergence proven, resumable bounded backfill with deterministic ids and no re-embedding, orphans recorded not guessed, 8 capability switches default-off, boolean-only credential status; 1658 offline tests, VERIFY: PASS. — spec: `docs/phase2.3/2.3.6-integration-migration-and-evaluation/2.3.6.1-schema-config-and-backward-compatible-migration.md` — add a small standard-library ordered/idempotent migration runner and `schema_migrations` table (securities/aliases/memberships → corpus items/sources/links → observations/events/actions → cursors/circuit state → indexes) introducing Phase 2.3 tables/config without losing existing SQLite/Chroma data or requiring every new source to be enabled at once.
