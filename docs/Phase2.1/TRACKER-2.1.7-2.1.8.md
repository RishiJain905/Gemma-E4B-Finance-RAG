...
- [x] 2.1.8.2 chat-client-performance-and-streaming — spec: `docs/Phase2.1/2.1.8-chat-tui-optimization-and-integration/2.1.8.2-chat-client-performance-and-streaming.md` — persistent client, /query/stream SSE (shared pipeline with /query, token events + terminal metadata incl. grounding), 404-fallback when tools on/model down, spinner, --timeout/--no-stream//verbose; live smoke: 163 tokens streamed, grounded, 4 citations. VERIFY: PASS (777).
- [x] 2.1.8.3 tui-integration-with-new-features — spec: `docs/Phase2.1/2.1.8-chat-tui-optimization-and-integration/2.1.8.3-tui-integration-with-new-features.md` — unified defensive renderer (grounding tag, tools_used, strategy, fetched-on-miss, resolved-ticker), /grounding //eval commands + hardened /tools, capabilities at startup, README + scripts/CHAT.md; manual E2E: analytical query via query_facts + projection query both render correctly. VERIFY: PASS (786).
